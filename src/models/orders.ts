...
export class Orders {
    ...
    async show(orderid: number): Promise<Order> {
        try {
            ...
            conn.release();
            if(!getOrder){
                throw new Error('Cannot find orders for this user or the user doesn\'t exist.')
            }
            let orderProducts = []
            for(const op of getOrder.rows){
                orderProducts.push({"Product": op.product, "pid": op.pid, "qty": op.quantity})
            }
        
            return {"userid": getOrder.rows[0].userid, "ostatus": getOrder.rows[0].ostatus, "ototal": getOrder.rows[0].ototal, "products": orderProducts}
        } catch(err) {
            throw new Error(`ERROR: ${err}`)
        }
    }
    ...
    async create(newOrder: Order): Promise<Order> {
        ...
        try {
            ...
            const createOrder = await conn.query(sqlQuery,[userid,ostatus,ototal])
            ...
            for(const op of products) {
                const addProducts = await conn.query(`INSERT INTO order_products VALUES ($1,$2,$3) RETURNING *`, [neworderid, op.pid, op.qty])
                newProducts.push(addProducts.rows[0])
                const getPrice = await conn.query(`SELECT pprice FROM products WHERE pid=($1)`, [op.pid])
                ordertotal += getPrice.rows[0].pprice*op.qty;
            }
            const orderCreated = await conn.query(`UPDATE orders SET ototal=($1) WHERE orderid=($2) RETURNING *`, [ordertotal, neworderid])
            conn.release();
            const response = {"userid": orderCreated.rows[0].userid, "ostatus": orderCreated.rows[0].ostatus, "products": newProducts, "ototal":orderCreated.rows[0].ototal}
            return  response
        } catch(err) {
    ...
    async currentOrder(userid: number,ostatus: string): Promise<Order> {
        try {
            ...
            const getProducts = await conn.query(`SELECT op.pid, op.pqnty from order_products op inner join products p on op.pid=p.pid where orderid=($1) ORDER BY orderid ASC`, [getOrder.rows[0].orderid])
            const products = getProducts.rows
            let productsList = [];
            for(const op of products){
                productsList.push({"pid": op.pid,"qty": op.pqnty})
            }
            return { "userid": getOrder.rows[0].userid, "ostatus": getOrder.rows[0].ostatus, "ototal": getOrder.rows[0].ototal ,"products": productsList}
        } catch(err) {
            throw new Error(`ERROR: ${err}`)
